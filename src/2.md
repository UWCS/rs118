...
**When writing a function, consider if you need to capture by value (move) or by reference**. If you don't need to modify the value, then always capture by reference instead of moving, like the `add_world()` and `calculate_length()` functions above do. If you need to modify the value but don't want to take ownership of it, then you need a...
