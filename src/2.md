...
Rust is a big fan of _zero-cost abstraction_, meaning that lots of these details that would ordinarily have to be done manually in C/C++, are abstracted away behind `String`'s implementation details. The zero-cost bit means its just as fast as doing it manually, so we get all this simplicity for free.
