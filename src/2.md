...
fn by_ref(s: &String) {
    ...
    //comment this line out to make this work
    s.push_str(" world");
...
fn main() {
    ...
    by_mut_ref(&mut s);
    by_value(s);
    //function above took ownership of s, cannot use
...
We can also use `impl` blocks to create **associated functions**, which are similar to methods, but don't capture `self`.
