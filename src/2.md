...
This is still dangerous. Remember that when variables go out of scope, they are `drop`ped and their memory is freed. If this were the case, the pointer would be freed twice, and this is a [big error](https://doc.rust-lang.org/nomicon/aliasing.html) that can cause the heap to be corrupted.
...
fn main() {
    ...
    // calculate_length has finished and ownership has reverted to s1
    println!("The length of '{}' is {}.", s1, len);
...
The compiler will ensure that our slice of the string remains valid (immutable). Uncommenting `s.clear()` fails as it creates a mutable reference to `s` (it borrows `s` mutably), which then invalidates our immutable slice in `word`, meaning we can no longer use it past that point. 
...
It's contiguous, heap allocated, dynamically-allocated, homogenous, linear, collection of elements.
...
We can also use `impl` blocks to create **associated functions**, which are similar to methods, but don't capture `self`. These are the equivalent of static functions from many languages.
