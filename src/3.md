...
Have a look at the error. Even though we annotated the lifetime, we're trying to return a reference to new memory, which will be dropped when the function ends. In these cases, it is better to use standalone types (`String`) instead of a reference.
