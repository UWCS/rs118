...
Generics in Rust are [monomorphised](https://en.wikipedia.org/wiki/Monomorphization) (same as C++ templates). At compile time, every time you use a generic type or function, a copy of it is made. If you have a function `foo<T>()`, and calls to `foo<String>()` and `foo<i32>()` are made in your program, there will be two copies of the function in your compiled binary, each with the type variable `T` replaced by the concrete types `String` and `i32`. [More on monomorphisation in rustc here](https://rustc-dev-guide.rust-lang.org/backend/monomorph.html)
