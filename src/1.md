...
- `cargo fmt` will format your code using [rustfmt](LINK)
- `cargo clippy` will lint your code using [clippy](LINK)
...
We also have floats, `f32` and `f64` which are IEEE754 floating point types.
...
I'll go into more detail on strings later.

#### Enums ([6.1](https://doc.rust-lang.org/book/ch06-01-defining-an-enum.html))

Enums are types that represent a finite set of discrete values, where the value taken by the enum is one of the values from the set. It's short for enumeration, because it is an enumeration of all the allowed options. Java and C both have support for enums, but Rust's enums are most similar to sum types in functional languages such has Haskell and F#.
...
The first two are lazily evaluated. If expressions return a value too, so we can:
...
There's no ternary conditional operator in Rust, because `if` expressions can do that.
