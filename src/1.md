...
If you want to loop over a numerical range, you can create a range iterator. Like Python's range, this is exclusive of the last number. However, unlike Python, you can clearly create a range thats *inclusive* of its last number:

```rust
for number in 1..10 {
    // this loop does not include the number 10
    println!("{}", number);
}

for number in 1..=10 {
    // this loop includes the number 10
    println!("{}", number);
}

for number in (1..10).rev() {
    // this loop goes from 9 -> 1
    println!("{}", number);
}
```
