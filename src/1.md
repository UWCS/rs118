...
Variables are **immutable by default**, meaning their value _cannot be changed_ once they have been bound. This is _A Good Thing™️_ because immutability means less potential for errors. Prefer to leave variables as immutable, unless you absolutely have to, in which case mutable variables can be declared `mut`:
...
- If a struct is bound as immutable, it's fields are also immutable.
...
println!("My student ID is: {}", joey.id);
joey.year +=1;
println!("My year of study is: {}", joey.year);
```

#### Enums ([6.1](https://doc.rust-lang.org/book/ch06-01-defining-an-enum.html))

Enums restrict variables to a predefined finite set of named discrete values. It's short for enumeration, because it is an enumeration of all the allowed options. Java and C both have support for enums, but Rust's enums are most similar to sum types in functional languages such has Haskell and F#.
...
Think of it as a type-safe box which may or may not be empty. This enum is frequently used as a return type in methods that may fail:
...
You all hopefully know how these work. The synatx is show below. you don't need parentheses around the condition in Rust.
...
Strings in Rust are [not so simple](https://dev-to-uploads.s3.amazonaws.com/uploads/articles/kqolzk0s0umg647d7ogg.jpeg). For now, we will consider only `String`. `String` represents a mutable, variable length buffer which can hold your `char`s. Create an empty one with `String::new()`, or create one from a literal using `String::from()`, or [`into`](https://doc.rust-lang.org/std/string/struct.String.html#impl-From%3C%26%27_%20str%3E)/[`to_owned`](https://doc.rust-lang.org/std/primitive.str.html#method.to_owned)

```rust, editable
let empty_string = String::new();
let hello_world = String::from("Hello, World!");
let hello_ferris: &str = "Hello, Ferris";
let owned_ferris: String = hello_ferris.to_owned();
...
We can also use it to destructure (unpack) and bind values, for use in the `match` body. We can use `match` to work with these `Option<T>`, for example:
...
```rust, editable
if let Some(value) = try_do_thing() {
    //do thing
} else {
    //handle error
}
