...
Enums are types that represent a finite set of discrete values, where the value taken by the enum is one of the values from the set. Java and C both have support for enums, but Rust's enums are most similar to sum types in functional languages such has Haskell and F#.
...
You all hopefully know how these work. Here's the syntax, you don't need parentheses around the condition in Rust:
...
Strings in Rust are [not so simple](https://dev-to-uploads.s3.amazonaws.com/uploads/articles/kqolzk0s0umg647d7ogg.jpeg). For now, we will consider only `String`. `String` represents a mutable, variable length buffer which can hold your `char`s. Create an empty one with `String::new()`, or create one from a literal using `String::from()`:
